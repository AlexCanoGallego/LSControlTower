...
const char KPAD_NOTES[12] = {
    4, 8, 12, 16, 20, 52, 56, 60, 64, 20, 26, 14
};

unsigned static char timerSms;
unsigned static char timerRebots;
unsigned static char state;
unsigned static char smsMode;

unsigned static char rowValues;

unsigned static char currentValue;
unsigned static char previousValue;

unsigned static char ready;
unsigned static char smsPressed;
unsigned static char pressCount;
...
void KPAD_Motor(void){
    ...
    switch(state){
        case 0:
            break;
        case 1:
            if(smsPressed && TI_GetTics(timerSms) >= 4000){
                smsPressed = 0;
                previousValue = 0xFF;
                ready = 1;
            }
            ...
            if(PORTCbits.RC0 == 1 || PORTCbits.RC1 == 1 || PORTCbits.RC3 == 1){
                ready = 0;
                state++;
        ...
        case 2:
            ...
            else if(TI_GetTics(timerRebots) >= 80){
                state++;
        ...
        case 3:
            ...
            currCol = (PORTC & 0x0B) >> 1;
            if(currCol == 4) currCol = 2;



            currRow = (PORTB & 0x0F) >> 1;
            if(currRow == 4) currRow--;
...
void escombra(void){
    rowValues <<= 1;
    if(rowValues == 0x10) rowValues = 0x01;
    PORTB = (PORTB & 0xF0) | rowValues;
}
