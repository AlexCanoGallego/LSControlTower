...
static unsigned char timerPWM, timerNote, timerMelody;
static unsigned char period, dutyCycle;
...
void BUZZ_Motor(void){
    ...
    switch (state){
        ...
        case 1:
            ...
            else if(melodyIndex >= 12){
                playingMelody = 0;
                state = 0;
            }
            break;
        case 2:
            if(TI_GetTics(timerNote) >= 400){
                TI_ResetTics(timerNote);
                state--;
                if(!playingMelody) state--;
                break;
            }
