...
# 5 "TAD_MENURECORDINGS.c" 2

# 1 "./TAD_SAVERECORDINGS.h" 1
...
struct Recording recordings[10];
unsigned char numRecordings;
...
# 9 "TAD_MENURECORDINGS.c" 2

# 1 "./TAD_BUZZER.h" 1
# 18 "./TAD_BUZZER.h"
const char MELODY[12] = {
    4, 12, 60, 8, 4, 60, 20, 60, 64, 20, 20, 14
};
...
static unsigned char option;
static unsigned char indexSent;
char strAux[12];
...
void MRCD_Motor(void){
    ...
    switch(state){
        ...
        case 1:
            ...
            break;
        case 2:
            if((aux = JOY_GetMove()) != 0){
                aux == 1 ? MRCD_OptionUp() : MRCD_OptionDown();
            }
            else if((aux = KPAD_GetChar()) == '#'){
                JOY_Disable();
                LCD_Clear();
        ...
        case 4:
            ...
            if(SIO_GetChar() == 'K') state++;
            break;
        case 5:
            if(!SIO_TXAvailable()) break;
            if(indexSent == 0 && recordings[option].id[indexSent] == 0){
                indexSent++;
            }
            else if(indexSent == 2){
                SIO_PutChar('\0');
                ...
                SIO_PutChar(recordings[option].id[indexSent++] + '0');
            }
            break;
        case 6:
